package com.makeappssimple.abhimanyu.espresso.android

import androidx.test.espresso.Espresso.onView
import androidx.test.espresso.Espresso.pressBack
import androidx.test.espresso.assertion.ViewAssertions.matches
import androidx.test.espresso.matcher.ViewMatchers.*
import androidx.test.ext.junit.rules.ActivityScenarioRule
import androidx.test.internal.runner.junit4.AndroidJUnit4ClassRunner
import org.junit.Rule
import org.junit.Test
import org.junit.runner.RunWith

@RunWith(AndroidJUnit4ClassRunner::class)
class SecondActivityTest {

    @get:Rule
    val activityRule = ActivityScenarioRule(SecondActivity::class.java)

    @Test
    fun test_isActivityInView() {
        onView(withId(R.id.layout_activity_second)).check(matches(isDisplayed()))
    }

    @Test
    fun test_visibilityOfTitleAndButton() {
        onView(withId(R.id.textview_activity_second_title)).check(matches(isDisplayed()))
    }

    @Test
    fun test_isCorrectTitleDisplayed() {
        onView(withId(R.id.textview_activity_second_title)).check(matches(withText(R.string.activity_second_title)))
    }
}
